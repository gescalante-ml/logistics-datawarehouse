...
def start(headers, data):
    ...
    for record in data:
        ...
        lane_id = ''.join([str(value) for key, value in truck_run['lane_key'].items()])

        # Updates the data structures
        if lane_id not in lanes:
            lanes[lane_id] = truck_run['lane_key']
    ...
    lanes = sql.get_lane_ids(lanes, insert_new=True)
    ...
    for truck_run in runs:
        ...
        lane_id = ''.join([str(value) for key, value in truck_run['lane_key'].items() if key != 'lane_key'])
        ...
        if lane_id in lanes:
            truck_run['lane_key'] = lanes[lane_id]['lane_key']
...
def transform(truck_run):
    ...
    truck_run = dimension.lane_dimension(truck_run)
